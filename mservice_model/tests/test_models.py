...
def test_calling_save_on_a_new_record_does_not_raise_error(
    mock_request, service, base_url
):
    ...
    mock_post.assert_called_once_with(
        "{}".format(base_url),
        json={"title": "Aloiba", "completed": False, "owner": None},
    )
    assert record.title == "Aloiba"
...
class SampleModelTestCase(base.BaseTestCase):
    ...
    def test_can_update_an_item(self):
        ...
        self.mock_put.return_value = MockRequest(
            {"id": 1, "title": "Buckle my shoe", "completed": True, "owner": None},
            overwrite=True,
        )
        SampleModel = model_factory(self.service, name="Sample")
        ...
        self.mock_put.assert_called_once_with(
            ...
            json={"completed": True, "owner": None, "title": "Buckle my shoe"},
        )
